...
neural_network_model = joblib.load('D:/New folder/Python/Data/DataBTL/neural_network_model.pkl')
...
# Chức năng kiểm tra dữ liệu đầu vào
def validate_input_data():
    try:
        ...
        # Kiểm tra điều kiện của dữ liệu đầu vào ở đây
        if age < 0 or sex < 0 or sex > 1 or chest_pain_type < 1 or chest_pain_type > 4 or resting_bp_s < 0 or cholesterol < 0 or fasting_blood_sugar < 0 or fasting_blood_sugar > 1 or resting_ecg < 0 or resting_ecg > 2 or max_heart_rate < 0 or exercise_angina < 0 or exercise_angina > 1 or oldpeak < 0 or ST_slope < 0:
...
def predict_diabetes_disease():
    if not validate_input_data():
        result_label.config(text='Dữ liệu đầu vào không hợp lệ')
        return
...
# Prediction button
predict_button = ttk.Button(root, text='Dự đoán', command=predict_diabetes_disease)
predict_button.grid(row=12, column=0, columnspan=2, padx=10, pady=5, sticky="we")

# Reset button
reset_button = ttk.Button(root, text="Thiết lập lại", command=reset_form)
